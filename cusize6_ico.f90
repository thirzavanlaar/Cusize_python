      ...
      real temp0,temp1(cmax),temp2(vmax,cmax),tempzf(kmax),tempzh(cmax,kmaxp1),xx0(3,cmax),xx1(3,cmax),xx2(3,cmax),test_area
      real*8 time,dayfrac
      ...
      fname_var = 'TA_dom4_qc_time42.nc'
      !fname_var = 'subsub_regular.nc'
      ...
      print *, 'min clon_vertices: ',minval(cell_vx)
...
!								  
      ...
      if (FIRST_CALL) then
        ...
        if (STATUS .ne. nf_noerr) call handle_err(STATUS)
        status = NF_DEF_DIM(ncidout,'max_nr_of_clouds',cmax/2,dimidcmax)
        if (STATUS .ne. nf_noerr) call handle_err(STATUS)
        ...
        dd(1) = dimidcmax
        status = NF_DEF_VAR(ncidout,'cloud_lon',nf_real,2,dd,varidlon)
        if (STATUS .ne. nf_noerr) call handle_err(STATUS)
        status = NF_DEF_VAR(ncidout,'cloud_lat',nf_real,2,dd,varidlat)
        if (STATUS .ne. nf_noerr) call handle_err(STATUS)
        status = NF_DEF_VAR(ncidout,'cloud_bin',nf_real,2,dd,varidbin)
        if (STATUS .ne. nf_noerr) call handle_err(STATUS)
        ...
        status = nf_put_att_text(ncidout, varidmean, 'long_name', 44, 'Mean shortest distance between clouds in bin')
        status = nf_put_att_text(ncidout, varidmean, 'units', 1, 'm')
        status = nf_put_att_text(ncidout, varidstdev, 'long_name', 61, 'Standard deviation of shortest distance between clouds in bin')
        ...
        status = nf_put_att_text(ncidout, varidd1, 'long_name', 43, 'Arithmetical mean distance of clouds in bin')
        status = nf_put_att_text(ncidout, varidd1, 'units', 1, 'm')
        status = nf_put_att_text(ncidout, varidlon, 'long_name', 25, 'Longitude of cloud centre')
        status = nf_put_att_text(ncidout, varidlon, 'units', 6, 'radian')
        status = nf_put_att_text(ncidout, varidlat, 'long_name', 24, 'Latitude of cloud centre')
        status = nf_put_att_text(ncidout, varidlat, 'units', 6, 'radian')
        status = nf_put_att_text(ncidout, varidbin, 'long_name', 21, 'Bin number of cloud')
        status = nf_put_att_text(ncidout, varidbin, 'units', 1, '-')
        status = nf_put_att_text(ncidout, varidncb, 'long_name', 23, 'Number of clouds in bin')
        status = nf_put_att_text(ncidout, varidncb, 'units', 1, '-')
      ...
      print *, 'min cloud_lon',minval(cloud_lon)
      status = NF_PUT_VARA_REAL(ncidout,varidlon,start2,count2,cloud_lon)
      if (STATUS .ne. nf_noerr) call handle_err(STATUS)
      status = NF_PUT_VARA_REAL(ncidout,varidlat,start2,count2,cloud_lat)
      if (STATUS .ne. nf_noerr) call handle_err(STATUS)
      status = NF_PUT_VARA_REAL(ncidout,varidbin,start2,count2,cloud_bin)
      if (STATUS .ne. nf_noerr) call handle_err(STATUS)
...
!								  
      ...
      call cloudsizes(nr2,m)

      print *, 'min val cloud_lon:',minval(cloud_lon)
      
      select case (iselect)
      case (1)
        call addtohist(nr2,m)
      case (2)
...
!      l(2,mm) = ((nrr*darea)/n)**0.5
      ...
      cloudcentre(mm,1) = (max_lon+min_lon)/2
      cloudcentre(mm,2) = (max_lat+min_lat)/2
      cloud_lon(mm) = (max_lon+min_lon)/2
      cloud_lat(mm) = (max_lat+min_lat)/2

      print *, 'cloud_lon in cloudsizes:',cloud_lon(mm)
...
!								  
      ...
      subroutine nnspacing()
 
      include 'parconst'
      include 'comvar'

      integer bb,cc,aa,ff,ss,pairs
      real firstx,firsty,secondx,secondy,mean,std
      ...
      do bb=1,nbin
        ...
        do aa = 1,ncloud !total nr of clouds
          if (cloudcentre(aa,3)==bb) then
            cc = cc+1
            binclouds(cc,1) = cloudcentre(aa,1)
            binclouds(cc,2) = cloudcentre(aa,2)
          endif
        enddo
